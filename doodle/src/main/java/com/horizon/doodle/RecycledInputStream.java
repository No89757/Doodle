...
/**
 * For {@link java.io.BufferedInputStream} in some version has some bug
...
class RecycledInputStream extends InputStream {
    ...
    RecycledInputStream(InputStream in) {
        this.in = in;
        buf = ByteArrayPool.INSTANCE.getBasicArray();
    }
    ...
    private void fill() throws IOException {
        ...
        if (markpos < 0)
            ...
            if (markpos > 0) {  /* can throw away early part of the buffer */
                ...
                if (nsz > marklimit)
                    nsz = marklimit;
                byte nbuf[] = ByteArrayPool.INSTANCE.getArray(nsz);
                System.arraycopy(buffer, 0, nbuf, 0, pos);
                ByteArrayPool.INSTANCE.recycleArray(buffer);
                buf = nbuf;
                buffer = nbuf;
            }
        count = pos;
    ...
    public void close() throws IOException {
        if(buf != null){
            ByteArrayPool.INSTANCE.recycleArray(buf);
            buf = null;
        }
